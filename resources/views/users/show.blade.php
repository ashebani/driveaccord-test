                ...
                <div class="relative flex flex-col items-center rounded-[10px] border-[1px] border-gray-200 w-full mx-auto p-4 bg-white bg-clip-border shadow-md shadow-[#F3F3F3] dark:border-[#ffffff33] dark:!bg-navy-800 dark:text-white dark:shadow-none">
                    ...
                    <div class="mt-6 mb-3 flex gap-14 md:!gap-14">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-sm font-normal text-gray-600">{{Str::plural('Post', $user->posts->count())}}</p>
                        </div>
                        <div class="flex flex-col items-center justify-center">
                            <p class="text-2xl font-bold text-navy-700 dark:text-white">
                                {{$user->comments->count()}}
                            </p>
                            <p class="text-sm font-normal text-gray-600">{{Str::plural('Comment', $user->comments->count())}}</p>
                        </div>
                        <div class="flex flex-col items-center justify-center">
                            <p class="text-2xl font-bold text-navy-700 dark:text-white">
                                {{$user->likes->count()}}
                            </p>
                            <p class="text-sm font-normal text-gray-600">{{Str::plural('Points', $user->likes->count())}}</p>
                        </div>
                    </div>
